...
function CreateApptModal({
  ...
  async function checkFollowUp(patientId: string) {
    if (!doctorOrgId) return
    const r = await checkFollowUpAction(patientId, doctorOrgId)
    setFollowUp(r)
  }
    ...
    start(async () => {
      if (mode === 'walkin') {
        ...
        const r = await registerWalkInAction({
          ...
          slot_end: slotEnd,
          patient_notes: wNotes || undefined,
        })
        if (r.error) { setError(r.error); return }
        setSuccess(`Walk-in registered. Queue #${r.queue_number}`)
        setTimeout(() => { onSuccess(); onClose() }, 1200)
      } else {
        if (!selectedPatient) { setError('Search and select a patient.'); return }
    ...
    <Modal title="New Appointment" onClose={onClose}>
      ...
      <div className="flex gap-2 mb-5">
        {(['walkin', 'existing'] as const).map(m => (
          ...
          >
            {m === 'walkin' ? '🚶 Walk-in' : '🔍 Existing Patient'}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Slot start</label>
            <input
              type="time"
              value={slotStart}
        ...
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1">Slot end</label>
          <input
            type="time"
            value={slotEnd}
          ...
          <div className="relative">
            ...
            {searchResults.length > 0 && (
              <div className="absolute z-10 top-full mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
                {searchResults.map((p: any) => (
                  ...
                  >
                    <p className="text-sm font-medium text-[#1A1A2E]">{p.full_name}</p>
            ...
            {selectedPatient && followUp && (
              <div className={`mt-2 px-3 py-2 rounded-xl text-xs font-medium ${
                ...
                {followUp.isFollowUp
                  ? `✓ Follow-up visit (last seen ${followUp.lastVisit})`
                  : 'New visit (no recent consultation found)'}
              </div>
            )}
            <div className="mt-3">
        ...
        >
          {isPending ? 'Saving…' : mode === 'walkin' ? 'Register Walk-in' : 'Create Appointment'}
        </button>
      </div>
    </Modal>
  )
