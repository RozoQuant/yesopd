...
export async function GET(request: NextRequest) {
  ...
  // Count all active statuses — exclude only CANCELLED and NO_SHOW.
  // Supabase returns `time` columns as "HH:MM:SS" (with seconds).
  // We normalize to "HH:MM" so keys match the slots generated by generateSlots.
  ...
  // Build booking count map: "date|HH:MM" → count
  // Normalize slot_start from "HH:MM:SS" → "HH:MM" to match generated slot keys
  const bookingCount = new Map<string, number>()
  for (const b of bookings ?? []) {
    const normalizedSlot = b.slot_start.slice(0, 5) // "13:00:00" → "13:00"
    const key = `${b.appt_date}|${normalizedSlot}`
  ...
  // ── 5. Get current time in minutes for filtering past slots on today ──
  const nowDate = new Date()
  const nowMins = nowDate.getHours() * 60 + nowDate.getMinutes()

  // ── 6. Build availability per date ────────────────────────
  const availability = []
  ...
  for (const d of dateRange) {
    ...
    combinedSlots.sort((a, b) => a.start.localeCompare(b.start))

    const dailyLimit =
      daySchedules.find(s => s.daily_limit != null)?.daily_limit ?? null
    ...
    availability.push({
      date: d,
      slots: dailyFull
        ? combinedSlots.map(s => ({ ...s, available: false }))
        : combinedSlots,
    })
  }
